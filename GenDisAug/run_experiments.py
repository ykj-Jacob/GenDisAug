...
DATASETS = {
    "sst2":      {"path": "SetFit/sst2",          "text_col": "text",     "test_split": "test"},
    "sst2_orig": {"path": "stanfordnlp/sst2",     "text_col": "sentence", "test_split": "validation"},
    "ag_news":   {"path": "ag_news",              "text_col": "text",     "test_split": "test"},
}


def get_balanced_subset(dataset, n_samples, seed, dataset_name):
    """Get balanced subset of training data."""
    ds_info = DATASETS[dataset_name]
    test_split = ds_info["test_split"]

    if n_samples == -1:
        return {"train": dataset["train"], "test": dataset[test_split]}
...
def run_bert_encoder(dataset_name, n_samples, seed, output_dir, augment=None):
    ...
    dataset = load_dataset(ds_info["path"])
    subset = get_balanced_subset(dataset, n_samples, seed, dataset_name)
    num_labels = len(set(subset["train"]["label"]))

    # Apply augmentation to training data
    train_texts = subset["train"][text_col]
    ...
    if augment:
        ...
        print(f"  Augmented from {len(subset['train'][text_col])} → {len(train_texts)} samples")
    ...
    tokenized["test"] = test_data.map(lambda x: tokenizer(x[text_col], padding="max_length", truncation=True, max_length=256), batched=True)
...
def run_gpt2_ar(dataset_name, n_samples, seed, output_dir, augment=None):
    ...
    # Apply augmentation
    train_texts_list = list(subset["train"][text_col])
    ...
    if augment:
        ...
        print(f"  Augmented from {len(subset['train'][text_col])} → {len(train_texts_list)} samples")
    ...
    model.eval()
    val_texts = subset["test"][text_col]
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="sst2", choices=["sst2", "sst2_orig", "ag_news"])
