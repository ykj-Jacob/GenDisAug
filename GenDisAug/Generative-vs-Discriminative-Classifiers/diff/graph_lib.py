...
class Absorbing(Graph):
    ...
    def dim(self):
        return self._dim + 1
        # print(f"This is the initial dimension {self._dim}")
        # return self._dim # modified by foobar # DO NOT FORGET TO CHANGE LATER
    ...
    def score_entropy(self, score, sigma, x, x0):
        ...
        # print(f"Score computation stats:")
        # print(f"- sigma: [{sigma.min().item()}, {sigma.max().item()}]")
        # print(f"- score range: [{score.min().item()}, {score.max().item()}]")
        
        rel_ind = x == self.dim - 1
        # print(f"Pad token matches: {(x == self.pad_token_id).sum()}")
        # print(f"rel_ind matches: {rel_ind.sum()}")
        # print(f"x is {x} and self.dim is {self.dim} and rel_ind is {rel_ind}")
        ...
        esigm1 = torch.where(
            sigma < 0.5,
            torch.expm1(sigma).clamp(min=eps),  # Prevent division by zero
            (torch.exp(sigma) - 1).clamp(min=eps)
        )
        ...
        # if torch.isnan(neg_term).any():
        #     print("NaN in neg_term")
        
        # if torch.isnan(pos_term).any():
        #     print("NaN in pos_term")
            
        # if torch.isnan(const).any():
        #     print("NaN in const term")
